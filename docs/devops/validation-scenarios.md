...
## Scenario 1 — Draft PR'da eksik PR body

- GitHub draft metadata'sı `true` olan PR body'de `Amaç` veya `Kapsam` boş bırakılır.
- Beklenen sonuç: PR Governance kontrolleri ertelemeyi notice olarak kaydeder ve terminal success üretir.
- Merge butonu: draft metadata ve branch ruleset nedeniyle kapalı.

## Scenario 2 — Ready PR'da eksik PR body
...
## Scenario 6 — PR gövdesinde taslak yazıyor ama metadata draft değil

- PR body'ye `Draft`, `Taslak` veya `WIP` yazılır fakat GitHub draft metadata kullanılmaz.
- Beklenen sonuç: `PR Governance / Body Validation` fail.
- Merge butonu: kapalı.

## Scenario 7 — Approval sonrası commit
