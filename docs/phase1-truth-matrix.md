...
| Attendance session lifecycle (M5) | #265 PR #346 (merged) | runtime | src/attendance: attendance-session.entity.ts, attendance-session.service.ts (+spec), attendance.controller.ts, teacher-own-lesson.guard.ts; 1824000000000-CreateAttendanceSessions.ts + 1825000000000-ReconcileAttendanceRecordsToSessionSchema.ts; test/database/create-attendance-sessions.migration.spec.ts; jest 10/10 PASS + tsc clean (7b9887a) | Product | P0 | controlled correction + locked-absence domain event (#266); fresh-DB UI journey (#269) |
...
- Attendance session lifecycle (#265): **merged to main** via PR #346 (squash merge `7b9887a`, 2026-09-21; PR head `40e8341`). PR #328 and PR #337 are CLOSED, superseded attempts — their branches were deleted (commits preserved as `refs/pull/328/head` and `refs/pull/337/head`). Immutable roster snapshot, teacher-own-lesson RBAC/BOLA and optimistic concurrency are runtime; controlled correction and locked-absence notification remain open (#266).
...
Current state: NOT merge-ready (notification/reporting/ux/acceptance open; attendance session lifecycle = `runtime` as of `7b9887a`).
