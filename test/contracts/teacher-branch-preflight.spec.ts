...
import { scanTeacherBranchPreflight, TEACHER_BRANCH_PREFLIGHT_REASONS } from '../../src/teachers/teacher-branch-preflight';
import { teacherBranchHappyPathFixture, teacherBranchNegativeFixture } from '../fixtures/m3/teacher-branch-preflight.fixture';
...
describe('TeacherBranch read-only preflight', () => {
  ...
  it('detects every canonical negative fixture and performs zero partial writes', () => {
    ...
    for (const reason of TEACHER_BRANCH_PREFLIGHT_REASONS) expect(result.countsByReason[reason]).toBeGreaterThan(0);
  });

  it('never chooses a default branch for ambiguous input', () => {
    const ambiguous = scanTeacherBranchPreflight(teacherBranchNegativeFixture).findings.find((finding) => finding.reasonCode === 'TB_AMBIGUOUS_BRANCH');
  ...
  it('exposes only opaque IDs, reason codes and counts', () => {
    const serialized = JSON.stringify(scanTeacherBranchPreflight(teacherBranchNegativeFixture));
  ...
  it('has no persistence or mutation surface in the scanner implementation', () => {
    const source = readFileSync(join(process.cwd(), 'src/teachers/teacher-branch-preflight.ts'), 'utf8');
    for (const forbidden of ['.save(', '.insert(', '.update(', '.delete(', '.remove(', '.softRemove(', 'QueryRunner', 'EntityManager', 'Repository<']) {
