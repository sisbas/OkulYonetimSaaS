(function attachPhaseOne(root, factory) {
  ...
  function operations(route, appState) {
    ...
    return `${pageHeading(route, 'Ders, izin, yoklama ve bilgilendirme istisnalarını tek günlük görünümde yönetin.', routeLink('/demo/f1/leaves/D-LV-204', 'İzin etkisini incele', 'button primary'))}
    ...
    <aside class="stack"><article class="panel"><div class="panel-header"><div><span class="section-kicker">Öncelik sırası</span><h3>Operasyon kuyruğu</h3></div></div><div class="attention-list">
      <a href="/demo/f1/leaves/D-LV-204" data-route class="attention-item ${metrics.unassignedLessons ? 'active' : 'done'}"><span aria-hidden="true">${metrics.unassignedLessons ? '1' : '✓'}</span><div><strong>İzin etkisi</strong><small>${metrics.unassignedLessons ? `${metrics.unassignedLessons} ders için yedek seçimi` : 'Demo kararı tamamlandı'}</small></div><b aria-hidden="true">→</b></a>
      <a href="/demo/f1/schedule?view=lifecycle" data-route class="attention-item ${appState.schedule.previewStatus === 'ready' ? 'active' : appState.schedule.previewStatus === 'accepted_demo' ? 'done' : ''}"><span aria-hidden="true">${appState.schedule.previewStatus === 'accepted_demo' ? '✓' : '2'}</span><div><strong>Program etkisi</strong><small>${appState.schedule.previewStatus === 'blocked' ? 'İzin kararı bekleniyor' : appState.schedule.previewStatus === 'ready' ? 'Önizleme doğrulanabilir' : 'Önizleme kabul edildi'}</small></div><b aria-hidden="true">→</b></a>
      <a href="/demo/f1/attendance/D-AT-1204" data-route class="attention-item ${appState.attendance.locked ? 'done' : ''}"><span aria-hidden="true">${appState.attendance.locked ? '✓' : '3'}</span><div><strong>Yoklama</strong><small>${appState.attendance.locked ? 'Demo oturumu tamamlandı' : 'Bir öğrenci durumu eksik'}</small></div><b aria-hidden="true">→</b></a>
      <a href="/demo/f1/notifications" data-route class="attention-item ${metrics.simulatedNotifications ? 'done' : ''}"><span aria-hidden="true">${metrics.simulatedNotifications ? '✓' : '4'}</span><div><strong>Veli bilgilendirme</strong><small>${metrics.simulatedNotifications ? 'Gönderim simülasyonu tamamlandı' : 'İnsan onayı gerekli'}</small></div><b aria-hidden="true">→</b></a>
  ...
  function leavesList(route, appState) {
    ...
    return `${pageHeading(route, 'Saatlik ve günlük izin taleplerini ders etkileriyle birlikte görün.', routeLink(`/demo/f1/leaves/${leave.id}`, 'Talebi aç', 'button primary'))}
    <article class="panel"><div class="table-header-row"><span>Talep</span><span>Öğretmen</span><span>Süre</span><span>Etkilenen ders</span><span>Durum</span><span></span></div><a class="table-data-row" href="/demo/f1/leaves/${leave.id}" data-route><strong>${esc(leave.id)}</strong><span>${esc(requester.label)}</span><span>${esc(leave.interval)}</span><span>${leave.affectedLessonIds.length}</span>${appState.leave.status === 'approved_demo' ? statusChip('Demo kararı tamamlandı', 'success') : statusChip('Karar bekliyor', 'warning')}<b aria-hidden="true">→</b></a></article>`;
  }

  function leaveDetail(route, appState, pathname) {
    const id = pathname.split('/').pop();
    const leave = leaveById(id);
    if (!leave) return `${pageHeading(route, 'İzin kararlarını yalnız tanımlı sentetik kayıtlar üzerinden inceleyin.', '')}${emptyState('!', 'Demo izin kaydı bulunamadı', `${id} kodu sentetik fixture içinde bulunmuyor.`, routeLink('/demo/f1/leaves', 'İzin Merkezine dön', 'button primary'))}`;
    const requester = teacherById(leave.requesterId);
    ...
    <aside class="stack"><article class="decision-card"><span class="decision-icon" aria-hidden="true">✓</span><h3>Yönetici kararı</h3><p>Uygunluk bilgisi kararı destekler; otomatik izin onayı oluşturmaz.</p>${approved ? `<div class="inline-success"><strong>Demo durumu güncellendi</strong><span>Gerçek izin kaydı veya görevlendirme oluşturulmadı.</span></div>${routeLink('/demo/f1/schedule?view=lifecycle', 'Program etkisini doğrula', 'button primary full')}` : `<button class="button primary full" data-approve-leave ${allAssigned ? '' : 'disabled'}>Onayı ve görevlendirmeyi simüle et</button><small class="validation-copy">${allAssigned ? 'Tüm dersler için uygun yedek seçildi.' : 'Devam etmek için üç dersin yedek öğretmenini seçin.'}</small>`}</article>
  ...
  function schedule(route, appState, search) {
    ...
    ].map(([id, label]) => routeLink(`/demo/f1/schedule?view=${id}`, label, `view-tab ${view === id ? 'active' : ''}`)).join('')}</nav>`;
  ...
  function notifications(route, appState) {
    ...
    ${simulated ? `<section class="success-banner final"><span aria-hidden="true">✓</span><div><strong>P0 operasyon akışı tamamlandı</strong><p>İzin, program, yoklama ve bilgilendirme adımları gerçek işlem oluşturmadan gösterildi.</p></div>${routeLink('/demo/f1/operations?status=complete', 'Tamamlanmış günü gör', 'button') }<button class="button primary" data-reset-scenario>Senaryoyu başa al</button></section>` : ''}`;
  }
